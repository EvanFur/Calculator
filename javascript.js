    document.addEventListener('DOMContentLoaded', function() {
        ...
        function appendOperator(op) {
            // If we're trying to add an operator but currentOperand is empty (after reset)
            if (resetScreen && operation !== null) {
              // Just change the operation, don't calculate
              operation = op;
              // Update the operator in the full equation
              const parts = fullEquation.split(' ');
              fullEquation = parts[0] + ' ' + operatorSymbols[op];
              updateDisplay();
              return;
            }
          
            if (operation !== null && !resetScreen) {
              // If we already have an operation and screen isn't reset, calculate the result first
              calculate(false);
            }
            
            operation = op;
            previousOperand = currentOperand + ' ' + operatorSymbols[op];
